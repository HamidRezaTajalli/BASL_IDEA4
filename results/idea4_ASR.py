...
def main():
        ...
        for model in models:
            ...
            for num_clients in num_clients_list:
                ...
                if not len(slctd_df['TEST_ACCURACY'].values) > 0 or not len(slctd_df['BD_TEST_ACCURACY'].values) > 0:
                    print(num_clients, dataset, model, alpha, cut_layer)
            ...
            line_handle = ax.errorbar(x=num_clients_list, y=clnums_ASR_list, marker=markers[1], alpha=0.8, markersize=4,
                        label=legend_color_labels[models.index(model)], linestyle=linestyles[1], color=colors[models.index(model)])
    ...
    plt.savefig(path_save.joinpath(f'idea4_{dataset}.pdf'))
    # plt.show()
